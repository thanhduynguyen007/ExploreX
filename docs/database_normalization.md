...
## 3. Các thiếu sót của SQL hiện tại

Từ `crebas5_fixed.sql`, các điểm chưa đủ cho triển khai thật:
...
1. Thêm `role` vào `Nguoidung`
2. Đổi `matKhau` thành `matKhauHash`
3. Thêm `maNguoiDung` vào `Nhacungcaptour`
